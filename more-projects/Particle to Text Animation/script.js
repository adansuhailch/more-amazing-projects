...
function init() {
    ...
    camera.position.z = window.innerWidth < 768 ? 32 : 25;
...
function createParticles() {
    ...
    function sphericalDistribution(i) {
        const phi = Math.acos(-1 + (3 * i) / count);
    ...
    for (let i = 0; i < count; i++) {
        ...
        color.setHSL(
            0.58 + depth * 0.1,0.8,0.45 + depth * 0.25 
        );
    ...
    const material = new THREE.PointsMaterial({
        size: 0.09,          // Thora bigger dots
        vertexColors: true,
        blending: THREE.AdditiveBlending,
        transparent: true,
        opacity: 0.95,       // Brighter galaxy
...
function createTextPoints(text) {
    ...
    for (let i = 0; i < pixels.length; i += 4) {
        if (pixels[i] > threshold) {
            ...
            if (Math.random() < 0.3) {
                const scale = isMobile ? (fontSize / 7) : (fontSize / 10);
...
function morphToCircle() {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const color = new THREE.Color();
        color.setHSL(
            0.58 + depth * 0.1,   // Blue → Cyan
            0.8,                 // Strong saturation
            0.45 + depth * 0.25 // Soft glow
        );
